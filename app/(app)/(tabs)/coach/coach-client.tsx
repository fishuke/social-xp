...
export function CoachClient({ name, prompt, locked, isPremium, history }: Props) {
  ...
  async function startRecording() {
    ...
    try {
      ...
      setError("The coach needs your mic — allow microphone access and try again.");
      return;
    }
  ...
  async function handleStopped(recordedType: string) {
    ...
    if (elapsed < MIN_SEC) {
      setPhase("ready");
      setBars(IDLE_BARS);
      setError("That was under 5 seconds — give it a little more room.");
      return;
    }
    ...
    try {
      ...
      setError("Something went wrong analyzing that one — try again.");
      setPhase("ready");
    }
    setBars(IDLE_BARS);
    setSeconds(0);
  ...
  if (lockedNow && phase !== "processing") {
    return <GatedScreen history={history} />;
  }
      ...
      {phase === "processing" ? (
        <div className="flex h-[150px] flex-col items-center justify-center gap-3">
          ...
          <p className="font-display text-[17px] font-semibold text-white">
            Coach is listening&hellip;
          </p>
          <p className="font-body text-[13px] font-bold text-ondark/60">
            Scoring your delivery — a few seconds.
          </p>
        </div>
      ) : (
        <div className="flex h-[150px] flex-col items-center justify-between pb-1">
          ...
          <p className="mt-2 font-body text-[13px] font-bold text-ondark/60">
            {recording ? "Tap to finish" : "Tap to start your rep"}
          </p>
        </div>
      )}
...
function FeedbackScreen({
  ...
  const gradeWord = a.overall >= 80 ? "Strong" : a.overall >= 60 ? "Solid" : "Brave";
  const metrics = [
    { label: "Confidence", value: a.scores.confidence },
    { label: "Clarity", value: a.scores.clarity },
    { label: "Energy", value: a.scores.energy },
  ];
    ...
    <div className="page-enter relative flex flex-1 flex-col pb-8">
      ...
      >
        <p className="font-body text-[12px] font-extrabold uppercase tracking-[1.5px] text-white/80">
          Your rep · {formatTime(result.durationSec)}
        </p>
      ...
      <div className="-mt-3 flex flex-col px-5">
        <section className="rounded-[20px] bg-white p-5 shadow-[0_2px_0_rgba(0,0,0,0.04)]">
            ...
            <div className="min-w-0 flex-1">
              ...
              {result.overallDelta !== null && result.overallDelta > 0 && (
                <p className="font-body text-[13px] font-bold text-go">
                  Up {result.overallDelta} points from last time
                </p>
              )}
            </div>
            {result.xpAwarded > 0 && (
          ...
          <p className="mt-4 font-body text-[14px] font-bold leading-[1.5] text-sec2">
            {a.summary} <span className="text-faint2">({a.wpm} wpm)</span>
          </p>
        </section>
        ...
        {lockedNow ? (
          <>
            <Link href="/paywall" className="btn btn-coral mt-5">
              Go unlimited with Social XP+
            </Link>
            <p className="mt-2 text-center font-body text-[13px] font-bold text-faint">
              That&apos;s today&apos;s free rep — new prompt tomorrow.
            </p>
          </>
        ) : (
          <>
        ...
        <HistoryList items={history} className="mt-6" />
      </div>
    </div>
  );
}
...
function GatedScreen({ history }: { history: CoachHistoryItem[] }) {
  return (
    <div className="page-enter flex flex-1 flex-col items-center px-6 pb-10 pt-[58px] text-center">
      ...
      <h1 className="mt-6 font-display text-[28px] font-semibold leading-[1.15] text-cocoa">
        Today&apos;s free rep — done. 💪
      </h1>
      <p className="mt-3 max-w-[300px] font-body text-[15px] font-bold leading-[1.5] text-sec2">
        Nice work showing up. A fresh prompt lands tomorrow — or go unlimited and rep as much as you
        want.
      </p>
      ...
      <HistoryList items={history} className="mt-8 w-full text-left" />
    </div>
  );
}
...
function HistoryList({ items, className }: { items: CoachHistoryItem[]; className?: string }) {
